...
$list_sub= json_decode($_POST['subKriteria'], true);
...
// $list_sub['Fasilitas'] = ["11", "12", "13", "14", "15", "16", "17"];
// $list_sub['Akademis'] = ["6", "7", "8", "9", "10"];
// $list_sub['Biaya'] = ["18", "19", "20", "21", "22"];
...
function get_table_subkriteria($kriteria, $list){
    require('connection.php');
    $list_subkriteria = $list[$kriteria];
    $arr_crit = array();
    for ($i=0; $i < sizeof($list_subkriteria); $i++) { 
        for ($j=0; $j < sizeof($list_subkriteria); $j++) { 
            ...
            if ($result2->num_rows > 0) {		
                while ($obj = $result2->fetch_assoc()) {
                    if($obj['sub_1'].'-'.$obj['sub_2'] == $list_subkriteria[$i].'-'.$list_subkriteria[$j]){                   
                        $arr_crit[$i][$j] = $obj['bobot'];               
                     }
                     else if($obj['sub_1'].'-'.$obj['sub_2'] == $list_subkriteria[$j].'-'.$list_subkriteria[$i] ){
                         $hasil = 1/$obj['bobot'];                
                         $arr_crit[$i][$j] = $hasil;                        
                     }                              
                }
...
function auto_data($jarak, $list_sekolah, $list_sub){
    require('connection.php');
    // require_once('proses_ahp_2.php');
...
$VE_ALT = array();
$autos = auto_data($jarak, $list_sekolah, $list_sub );
//console_log($autos);
